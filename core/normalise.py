...
from core.containers import total_for
...
def normalise_container_count(value: str) -> "int | None":
    ...
    Documents write "6 x 40'HC". The real object here is a COMPOSITION - how
    many of each size - not a bare total, so parsing and the summing/mixed-
    equipment decision now live in core.containers (see that module's
    docstring for the full ladder and why summing a mixed shipment is only
    safe because core.compare gates it on the group breakdown afterwards).

    This function keeps its old signature (str in, int-or-None out) and just
    delegates to core.containers.total_for - exactly how normalise_weight
    delegates to core.units.to_kilograms. A wrong MATCH is still the
    expensive error here, so the underlying parser fails closed to None
    (undecidable, per core/compare.py) rather than guess whenever the
    quantity itself is ambiguous (a SIZE-first form like "40' x 2").
    """
    text = _as_text(value)
    if text is None:
        return None
    return total_for(text)
